...
// Assuming 'api' is an axios-like utility for demonstration
// If you are running this code, you might need to mock or define api.
const api = {
    get: async (url) => {
        // Mock API Data
        const mockImpacts = [
            { id: 1, level: "Low", sortOrder: 3, active: true, isDefault: false },
            { id: 2, level: "Medium", sortOrder: 2, active: true, isDefault: false },
            { id: 3, level: "High", sortOrder: 1, active: true, isDefault: true },
            { id: 4, level: "Testing", sortOrder: 4, active: false, isDefault: false },
        ];
        return { data: mockImpacts };
    },
    put: async (url, data) => ({ data }),
    post: async (url, data) => ({ data: { ...data, id: Date.now() } }),
    delete: async (url) => ({ data: {} }),
};


// --- Utility Icons (kept as is) ---
const PlusIcon = (props) => (
  <svg {...props} xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLineLinejoin="round" className="w-5 h-5">
    <path d="M5 12h14" />
    <path d="M12 5v14" />
  </svg>
);
...
// --- Custom Modal Components (New/Improved) ---
...
const ConfirmModal = ({ message, onConfirm, onCancel }) => {
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-75 flex justify-center items-center z-[60] p-4 transition-opacity duration-300">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6 transform transition-transform duration-300 scale-100">
                <h3 className="text-xl font-bold mb-4 text-gray-800 border-b pb-2">Confirm Deletion</h3>
...
/**
 * Modal for Adding/Editing Impact Levels
 */
const ImpactModal = ({ currentImpact, handleSave, setIsModalOpen }) => {
...
  };
  ...
  // **FIX APPLIED: Replaced window.confirm with custom modal logic**
  const handleDelete = (id) => {
    setConfirmMessage(`Are you sure you want to permanently delete Impact ID ${id}? This action cannot be undone.`);
...
  const filteredImpacts = impacts.filter(i => i.active === true || includeInactive);
...
            <h2 className="text-2xl font-bold text-gray-800 uppercase">Impact List Management</h2>
